...
- `core/config.py` - settings load/save (JSON in `%APPDATA%\VortexTorrent\settings.json`), default download dir, validated/clamped load + corrupt-file backup
...
- `updater.py` - GitHub release check/download with SHA-256 checksum + Authenticode signature verification + newer-only guard (never downgrades), cached-signed-installer reuse, stale installer cleanup
